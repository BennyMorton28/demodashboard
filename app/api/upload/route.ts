...
export default function ${componentName}Demo() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-white">
      ...
      <header className="h-16 border-b border-gray-200 relative z-50 bg-white flex-shrink-0">
        <div className="container mx-auto h-full px-6">
          <div className="flex items-center justify-between h-full">
            ...
            <h1 className="text-2xl font-bold text-gray-900">${demoTitle}</h1>
      ...
      <main className="flex-1 flex overflow-hidden">
        {/* Left side - Assistant */}
        <div className={\`
          md:w-1/2 w-full border-r border-gray-200 overflow-hidden
          \${activeTab === 'chat' ? 'block' : 'hidden md:block'}
        \`}>
          <${assistantComponentName} />
        </div>
        
        {/* Right side - Demo Markdown Display */}
        <div className={\`
          md:w-1/2 w-full overflow-hidden
          \${activeTab === 'content' ? 'block' : 'hidden md:block'}
        \`}>
          <DemoMarkdownDisplay demoId="${demoId}" />
        </div>
      </main>

      {/* Footer */}
      <footer className="h-auto min-h-12 border-t border-gray-200 bg-white relative z-50 py-3 flex-shrink-0">
